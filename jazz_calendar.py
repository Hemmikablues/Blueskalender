...
def parse_katalin_page(html: str, url: str, today: date | None = None) -> Event | None:
    ...
    Katalin exposes date, stage time and venue as labelled fields on each event page.
    JSON-LD is preferred when present, with the visible labels as a robust fallback.
    """
    today = today or date.today()
    soup = BeautifulSoup(html, "html.parser")
    text = soup_text(soup)

    structured = parse_jsonld_event(soup, "Katalin", url)
    if structured:
        structured.source = "Katalin"
        structured.city = structured.city or "Uppsala"
        structured.address = structured.address or "Roslagsgatan 1, 753 27 Uppsala"
        # Katalin's structured location can be generic. Prefer the explicit Scen field.
        venue_m = re.search(r"Musikgenre\s*:.*?\s+Scen\s*:\s*(.+?)(?=\s+Bordsbokning\s*:|\s+Ståplats\s*:|\s+Biljettpris\s*:|$)", text, re.I)
        if venue_m:
            structured.venue = clean_text(venue_m.group(1))
        structured.venue = structured.venue or "Katalin"
        cat_m = re.search(r"Musikgenre\s*:\s*(.+?)(?=\s+Scen\s*:|$)", text, re.I)
        if cat_m:
            structured.category = clean_text(cat_m.group(1))
        return structured

    d = parse_swedish_date(text, today)
    tm = re.search(r"På\s+scen\s*:\s*([0-2]?\d[:.]\d{2})", text, re.I)
    if not d or not tm:
        return None
    start_t = parse_hhmm(tm.group(1))
    if not start_t:
    ...
    return Event(
        title=title_from_soup(soup),
        start=start,
        end=sensible_end(start),
        venue=clean_text(venue_m.group(1)) if venue_m else "Katalin",
        address="Roslagsgatan 1, 753 27 Uppsala",
        city="Uppsala",
        source="Katalin",
        url=url,
        description=desc,
...
def scrape_katalin(session: requests.Session, today: date) -> list[Event]:
    """Read all paginated future events from Katalin and parse their detail pages."""
    base = SOURCES["Katalin"]
    links: list[str] = []
    seen: set[str] = set()
    stagnant_pages = 0

    for page in range(1, 16):
        page_url = base if page == 1 else set_query(base, tab=str(page))
        try:
            soup = BeautifulSoup(fetch(session, page_url, timeout=20).text, "html.parser")
        except Exception as exc:
            if page == 1:
                raise
            logging.debug("Katalin page %s failed: %s", page, exc)
            break
        new_count = 0
        for a in soup.find_all("a", href=True):
            href = urljoin(page_url, a["href"]).split("#", 1)[0]
            parsed = urlparse(href)
            if parsed.netloc not in ("www.katalin.com", "katalin.com"):
                continue
    ...
    if not links:
        raise SourceError("Katalins kalendarium gav inga evenemangslänkar.")
    ...
    for link in links[:220]:
        try:
            ...
            r.raise_for_status()
            ev = parse_katalin_page(r.text, link, today)
            if ev:
                events.append(ev)
        except Exception as exc:
            logging.debug("Katalin event failed %s: %s", link, exc)
    if not events:
        raise SourceError("Katalins evenemangssidor kunde inte tolkas.")
    return deduplicate(events)


def _month_tokens(month: int) -> str:
    variants = {
        1: "jan(?:uari)?", 2: "feb(?:ruari)?", 3: "mar(?:s)?", 4: "apr(?:il)?",
        5: "maj", 6: "jun(?:i)?", 7: "jul(?:i)?", 8: "aug(?:usti)?",
        9: "sep(?:t(?:ember)?)?", 10: "okt(?:ober)?", 11: "nov(?:ember)?", 12: "dec(?:ember)?",
    }
    return variants[month]


def _jazzoblues_time_for_date(text: str, d: date) -> tuple[time | None, time | None]:
    """Find the exact Facebook-embedded time for a date when it is present on the page."""
    mon = _month_tokens(d.month)
    pat = rf"\b0?{d.day}\s+{mon}\s+{d.year}\s*,\s*([0-2]?\d[:.]\d{{2}})(?:\s*-\s*(?:0?{d.day}\s+{mon}\s+{d.year}\s*,\s*)?([0-2]?\d[:.]\d{{2}}))?"
    m = re.search(pat, text, re.I)
    if not m:
        return None, None
    return parse_hhmm(m.group(1)), parse_hhmm(m.group(2)) if m.group(2) else None


def _best_anchor_url(soup: BeautifulSoup, title: str, base: str) -> str:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not label:
            continue
        score = SequenceMatcher(None, nt, normalize_title(label)).ratio()
        if score > best_score and score >= 0.82:
            best_score = score
            best_url = urljoin(base, a["href"])
    return best_url


def scrape_jazzoblues_kungsbacka(session: requests.Session, today: date) -> list[Event]:
    """Parse Jazz & Blues i Kungsbacka's published programme and Blues Jam dates."""
    url = SOURCES["Jazz & Blues i Kungsbacka"]
    html = fetch(session, url, timeout=20).text
    soup = BeautifulSoup(html, "html.parser")
    text = soup_text(soup)
    lines = [clean_text(x) for x in soup.stripped_strings if clean_text(x)]
    events: list[Event] = []

    # Main autumn/spring programme. The site's list uses: Title, then "Weekday D Month – Venue".
    start_idx = next((i for i, x in enumerate(lines) if re.search(r"Konserter\s+(?:höst|vår)", x, re.I)), -1)
    if start_idx >= 0:
        stop_idx = next((i for i in range(start_idx + 1, len(lines)) if lines[i].lower().startswith("mer info kommer")), len(lines))
        date_re = re.compile(rf"^{WEEKDAY_RE}\s+(\d{{1,2}})\s+({MONTH_RE})(?:\s+(\d{{4}}))?\s*[–—-]\s*(.+)$", re.I)
        noise = re.compile(r"^(?:biljetter|info här|boka|köp|läs mer|konserter\b)", re.I)
        for i in range(start_idx + 1, stop_idx):
            ...
            for j in range(i - 1, start_idx, -1):
                candidate = lines[j]
                if date_re.match(candidate) or noise.search(candidate):
                    continue
                title = candidate
                break
            if not title:
                continue
            venue = clean_text(m.group(4))
            start_t, end_t = _jazzoblues_time_for_date(text, d)
            if not start_t:
                if "teater" in venue.lower():
                    start_t = time(19, 30)
                elif "stadsfestival" in title.lower():
                    start_t = time(14, 0)
                else:
                    start_t = time(19, 0)
            start = local_dt(d, start_t)
            if "teater" in venue.lower():
                ...
                address = "Norra Torggatan 4, 434 30 Kungsbacka"
            events.append(Event(
                title=title,
                start=start,
                end=sensible_end(start, end_t),
                venue=venue or "Restaurang Ester",
                address=address,
                city="Kungsbacka",
                source="Jazz & Blues i Kungsbacka",
                url=_best_anchor_url(soup, title, url),
            ))

    # Blues Jam is one recurring event with several dates. Prefer the embedded event times,
    # since they also correct occasional prose-date typos on the page.
    jam_pos = text.lower().find("blues jam open stage")
    if jam_pos >= 0:
        jam_block = text[jam_pos:jam_pos + 1500]
        jam_re = re.compile(r"\b(\d{1,2})\s+(jan|feb|mar|apr|maj|jun|jul|aug|sep|okt|nov|dec)\s+(20\d{2})\s*,\s*14[:.]00", re.I)
        seen_dates: set[date] = set()
        for m in jam_re.finditer(jam_block):
            month = MONTHS[m.group(2).lower()]
            try:
            ...
            if d in seen_dates:
                continue
            seen_dates.add(d)
            start = local_dt(d, time(14, 0))
            events.append(Event(
                title="Blues Jam Open Stage",
                start=start,
                end=local_dt(d, time(16, 30)),
                venue="Restaurang Ester",
                address="Norra Torggatan 4, 434 30 Kungsbacka",
                city="Kungsbacka",
                source="Jazz & Blues i Kungsbacka",
                url=_best_anchor_url(soup, "Blues Jam Open Stage", url),
                description="Blues Jam / Open Stage. Fri entré enligt arrangörens program.",
            ))

    if not events:
        raise SourceError("Jazz & Blues i Kungsbacka gav inga tolkbara framtida evenemang.")
    return deduplicate(events)
...
def write_outputs(events: list[Event], statuses: list[SourceStatus], output_dir: Path) -> None:
    ...
    uppsala = [e for e in events if e.city.lower() == "uppsala"]
    kungsbacka = [e for e in events if e.city.lower() == "kungsbacka"]
    ...
    (output_dir / "uppsala.ics").write_text(calendar_text(uppsala, "Musikkalender, Uppsala"), encoding="utf-8", newline="")
    (output_dir / "kungsbacka.ics").write_text(calendar_text(kungsbacka, "Musikkalender, Kungsbacka"), encoding="utf-8", newline="")
